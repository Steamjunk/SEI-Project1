...
class GameObject {
    ...
    setupBoard() {
        ...
        //must be removed during reset
        this.gameStacks[0].addEventListener('click', event => {
            document.querySelector('#difficulty').addEventListener('change', 
                function difficultyDetector() {
                    event.preventDefault();
                    console.log(this);
                    this.removeEventListener('change', difficultyDetector);
                });

            this.startTimer()}, {once : true}
        );
    ...
    resetGame() {
        ...
        const newStack = this.gameStacks[0].cloneNode();
        this.gameStacks.shift();
        this.gameStacks.unshift(newStack);
    }
